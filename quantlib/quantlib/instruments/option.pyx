...
cdef class VanillaOption:
    ...
    def __init__(self, Payoff payoff, Exercise exercise):
        ...
        cdef shared_ptr[_payoffs.StrikedTypePayoff]* payoff_ptr = \
                new shared_ptr[_payoffs.StrikedTypePayoff](<_payoffs.StrikedTypePayoff*>payoff._thisptr)

        cdef shared_ptr[_exercise.Exercise]* exercise_ptr = new shared_ptr[_exercise.Exercise](exercise._thisptr)

        self._thisptr = new _option.EuropeanOption(
            deref(payoff_ptr), deref(exercise_ptr)
        )
